      ...
      <div className="flex bg-slate-100 p-1 rounded-xl mb-5 md:mb-6">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 min-h-[44px] py-2.5 text-sm font-semibold rounded-lg transition-all duration-200 cursor-pointer touch-manipulation
            ${config.mode === 'frame' ? 'bg-white text-signal shadow-md' : 'text-slate-600 hover:text-slate-900'}`}
        ...
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 min-h-[44px] py-2.5 text-sm font-semibold rounded-lg transition-all duration-200 cursor-pointer touch-manipulation
            ${config.mode === 'sheet' ? 'bg-white text-signal shadow-md' : 'text-slate-600 hover:text-slate-900'}`}
      ...
      <div className="space-y-5 md:space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {config.mode === 'frame' ? (
            ...
            <>
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1.5 flex justify-between">
                  {t.gridCols} <span className="text-signal font-bold">{config.gridCols}</span>
                </label>
                <input
                  type="range"
                  min="2"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1.5 flex justify-between">
                  {t.gridRows} <span className="text-signal font-bold">{config.gridRows}</span>
                </label>
                <input
                  type="range"
                  min="1"
        ...
        {config.mode === 'sheet' && (
          <div className="space-y-2">
            <div className="text-xs text-signal bg-signal-soft p-3 rounded-lg border border-signal/25 flex items-center gap-2 shadow-sm">
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 min-h-[44px] py-2 px-3 rounded-lg text-xs font-semibold transition-all cursor-pointer border-2 touch-manipulation ${
                    config.chromaKeyColor === 'magenta'
                      ...
                      : 'border-slate-200 bg-white text-slate-600 hover:border-[#E879F9]'
                  }`}
                  aria-label={t.magentaColor}
                >
                  <div className="w-4 h-4 rounded-full bg-[#E879F9] border border-fuchsia-600 flex-shrink-0"></div>
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 min-h-[44px] py-2 px-3 rounded-lg text-xs font-semibold transition-all cursor-pointer border-2 touch-manipulation ${
                    config.chromaKeyColor === 'green'
                      ...
                      : 'border-slate-200 bg-white text-slate-600 hover:border-ok/40'
                  }`}
                  aria-label={t.greenScreen}
                >
                  <div className="w-4 h-4 rounded-full bg-[#22C55E] border border-green-600 flex-shrink-0"></div>
            ...
            <div className="flex items-center justify-between p-3 rounded-lg border border-slate-200 bg-slate-50">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                ...
                />
                <div className="w-9 h-5 bg-slate-300 peer-focus:outline-none peer-focus:ring-2 peer-focus-visible:ring-signal rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-signal"></div>
              </label>
            </div>
              ...
              {(MODEL_RESOLUTIONS[selectedModel] ?? ['1K']).map((res) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium border-2 transition-colors ${
                    selectedResolution === res ? 'border-signal bg-signal-soft text-signal' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300'
                  }`}
                >
                  {res}
                </button>
        ...
        <button
          ...
          className={`w-full min-h-[48px] py-3.5 rounded-xl font-bold text-white shadow-md flex items-center justify-center gap-2 transition-all duration-200 cursor-pointer touch-manipulation tap-highlight
            ${
              isGenerating
                ? 'bg-signal/70 cursor-not-allowed opacity-75'
                : 'bg-signal hover:bg-signal-hover hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0'
            }`}
