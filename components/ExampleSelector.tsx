      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div className="text-left">
            <h3 className="text-sm font-semibold text-orange-900">範例提示詞</h3>
            <p className="text-xs text-orange-700">點擊選擇預設的動畫範例</p>
          </div>
        </div>
        {isExpanded ? (
        ...
        <div className="px-4 pb-4">
                ...
                <div className="mt-2 pt-2 border-t border-orange-100">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-500">
                      {example.gridCols * example.gridRows} 幀
                    </span>
                    <span className={`px-2 py-0.5 rounded-full text-white text-[10px] font-medium ${
                      ...
                      {example.chromaKeyColor === 'magenta' ? '洋紅背景' : '綠幕背景'}
                    </span>
                  </div>
                </div>
              </button>
          ...
          <div className="mt-3 p-2.5 bg-orange-100/50 rounded-lg border border-orange-200">
            <p className="text-xs text-orange-800 leading-relaxed">
              ...
              選擇範例後，系統會自動填入提示詞、設定網格大小和背景色。您可以根據需要修改這些設定。
            </p>
          </div>
        </div>
      )}
